...
// -----------------------------------------------------------------------
// 2. Навигация прячется при скролле вниз и появляется при скролле вверх
//    (меню больше не сворачивается в бургер — на мобильных ссылки просто
//    прокручиваются горизонтально внутри "таблетки").
// -----------------------------------------------------------------------
...
if (navbar) {
  ...
  window.addEventListener("scroll", () => {
    ...
    requestAnimationFrame(() => {
      ...
      if (currentY < 80) {
        navbar.classList.remove("nav-hidden"); // у самого верха меню всегда видно
      } else if (currentY > lastScrollY) {
        navbar.classList.add("nav-hidden"); // скролл вниз — прячем
      } else {
        navbar.classList.remove("nav-hidden"); // скролл вверх — показываем
      }
